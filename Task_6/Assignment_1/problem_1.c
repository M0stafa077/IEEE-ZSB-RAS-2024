...
int main()
{
    ...
    /* Reallocating the space allocated by calloc to 30 Integers using realloc() */
    fourth = (unsigned int *) realloc(third, REALLOC_SIZE * sizeof(int));
    if(NULL == fourth)
        { printf("Error allocating memory using realloc() function\n"); return 1; }
    else
        { printf("Re-allocated memory for %d Integers using realloc() function\n", REALLOC_SIZE); }
    
    printf("Values Assigned to the allocated memory: \n");
    for(l_counter = 0; l_counter < REALLOC_SIZE; ++l_counter)
        { printf("%d ", fourth[l_counter]); }
    printf("\nZeros for the old locations\nGarbage values for the new allocated locations\n");
    printf("=======================================================\n");
