...
const uint8_t buttons_values[KEYPAD_ROWS][KEYPAD_COLUMNS] = 
{
    {'7', '8', '9', '/'},
    {'4', '5', '6', '*'},
    {'1', '2', '3', '-'},
...
Std_ReturnType keypad_Start_With_Interrupts(keypad_t* _keypad)
{
	...
	else
	{
		...
		for(l_rows_counter = ZERO_INIT; l_rows_counter < KEYPAD_ROWS; l_rows_counter++)
		{
			...
			for(l_counter = ZERO_INIT;l_counter < KEYPAD_ROWS; ++l_counter)
			{
				retVal |= gpio_pin_write_logic(&(_keypad->keypad_row_pins[l_counter]), GPIO_LOW);
			}
			
			/* Then write HIGH on the current row */
			retVal |= gpio_pin_write_logic(&(_keypad->keypad_row_pins[l_rows_counter]), GPIO_HIGH);
		}
	}
	return retVal;
}